    ...
    <>
      <style jsx global>{`
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        body { background: #080202; overflow-x: hidden; }
      `}</style>
      ...
      <style>{`
        ...
        .embers { position: absolute; inset: 0; z-index: 2; overflow: hidden; pointer-events: none; }
        .ember {
          position: absolute;
        ...
        .back-button:hover { color: #cc1a1a; transform: translateX(-4px); }
        ...
        .card-title em { font-style: normal; color: #cc1a1a; }
        .card-subtitle { font-family: 'Crimson Pro', serif; font-size: 0.9rem; color: #8a6060; letter-spacing: 0.06em; }

        .login-form { display: flex; flex-direction: column; gap: 1.5rem; }
        .form-group { display: flex; flex-direction: column; gap: 0.5rem; animation: slideIn 0.5s ease backwards; }
        ...
        .form-label { font-family: 'Cinzel', serif; font-size: 0.7rem; letter-spacing: 0.2em; color: #d4c4c4; text-transform: uppercase; }
        .input-wrapper { position: relative; display: flex; align-items: center; }
        .input-icon { position: absolute; left: 1rem; font-size: 1rem; opacity: 0.6; pointer-events: none; }
        .form-input {
          width: 100%;
        ...
        .form-input:focus { outline: none; border-color: #cc1a1a; box-shadow: 0 0 10px rgba(200,30,30,0.3); }
        .form-input::placeholder { color: #4a3030; font-style: italic; }
        .input-error { border-color: #cc1a1a !important; box-shadow: 0 0 8px rgba(200,30,30,0.4); }
        .error-message { color: #cc1a1a; font-family: 'Crimson Pro', serif; font-size: 0.75rem; margin-top: 0.25rem; }
        .success-message { color: #4caf50; font-family: 'Crimson Pro', serif; font-size: 0.75rem; text-align: center; }

        .forgot-password { text-align: right; margin-top: -0.5rem; }
        .forgot-link { font-family: 'Crimson Pro', serif; font-size: 0.7rem; color: #8a6060; cursor: pointer; background: none; border: none; transition: color 0.3s; }
        .forgot-link:hover { color: #cc1a1a; }
        ...
        .btn-login::before { content:''; position:absolute; inset:0; background:linear-gradient(135deg,#cc1a1a 0%,#8b0000 100%); opacity:0; transition:opacity 0.3s; }
        .btn-login::after  { content:''; position:absolute; top:0; left:-100%; width:60%; height:100%; background:linear-gradient(90deg,transparent,rgba(255,255,255,0.12),transparent); transition:left 0.5s; }
        .btn-login:hover:not(:disabled)::before { opacity: 1; }
        .btn-login:hover:not(:disabled)::after  { left: 150%; }
        .btn-login:hover:not(:disabled) { box-shadow: 0 0 30px rgba(200,20,20,0.7); border-color: #ff3333; transform: translateY(-2px); }
        .btn-login:disabled { opacity: 0.7; cursor: not-allowed; }
        .btn-content { position:relative; z-index:1; display:flex; align-items:center; justify-content:center; gap:0.75rem; }

        .corner { position:absolute; width:8px; height:8px; border-color:#c9a84c; border-style:solid; opacity:0.6; z-index:1; }
        ...
        .divider::before, .divider::after { content:''; flex:1; border-bottom:1px solid rgba(100,20,20,0.3); }
        .divider span { font-family:'Crimson Pro',serif; font-size:0.7rem; color:#8a6060; padding:0 0.75rem; letter-spacing:0.1em; }

        .social-login { display:flex; flex-direction:column; gap:1rem; }
        .social-btn {
          display:flex; align-items:center; justify-content:center;
          font-family:'Cinzel',serif; font-size:0.75rem; font-weight:600;
          letter-spacing:0.15em; text-transform:uppercase;
          gap:0.75rem; padding:0.875rem; cursor:pointer;
          transition:all 0.3s ease;
          border:1px solid rgba(100,20,20,0.5);
          background:rgba(8,2,2,0.8);
          color:#d4c4c4;
        ...
        .register-link { text-align:center; margin-top:2rem; padding-top:1.5rem; border-top:1px solid rgba(100,20,20,0.3); }
        .register-link p { font-family:'Crimson Pro',serif; font-size:0.85rem; color:#8a6060; }
        .link-highlight { color:#cc1a1a; text-decoration:none; font-weight:600; transition:color 0.3s; }
        .link-highlight:hover { color:#ff3333; text-decoration:underline; }

        .security-seal { display:flex; align-items:center; justify-content:center; gap:0.5rem; margin-top:1.5rem; padding-top:1rem; }
        .seal-icon { font-size:1rem; opacity:0.6; }
        .security-seal p { font-family:'Crimson Pro',serif; font-size:0.7rem; color:#4a3030; letter-spacing:0.05em; }

        @media (max-width: 640px) {
          .login-container { padding: 1rem; }
          .login-card { padding: 2rem 1.5rem; }
          .card-title { font-size: 1.8rem; }
          .btn-login { font-size: 0.7rem; padding: 14px 24px; }
        }
      `}</style>
      ...
      <div className="login-container">
        ...
        <div className="embers" aria-hidden="true">
          {Array.from({ length: 12 }).map((_, i) => <div key={i} className="ember" />)}
        </div>
        ...
        <div className="login-content">
          ...
          <div className="login-card">
            ...
            <div className="card-header">
              ...
              <h1 className="card-title">Invoque <em>Sua Conta</em></h1>
              <p className="card-subtitle">O Nexus Carmesim aguarda seu retorno, guerreiro</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="login-form">
              ...
              <div className="forgot-password">
                <button type="button" onClick={handleResetPassword} className="forgot-link" disabled={isLoading}>
              ...
              <button type="submit" className="btn-login" disabled={isLoading}>
                ...
                <span className="btn-content">
                  {isLoading
                    ? <><span className="loading-spinner" /> Conjurando acesso...</>
                    : "Entrar no Nexus"
                  }
                </span>
              </button>
            ...
            <div className="divider"><span>ou conecte-se com</span></div>
            ...
            <div className="register-link">
              <p>Novo no reino?{" "}<Link href="/register" className="link-highlight">Forje seu destino</Link></p>
            </div>
