...
type RosterMember = { user_id: string; character_id: string | null; nome: string | null; foto_url: string | null; avatar: string | null; vida: string; sanidade: string };
...
function CharacterPortrait({ member, cameraLive = false }: { member: Member; cameraLive?: boolean }) {
  ...
  return <div className={`live-character-portrait ${cameraLive ? "camera-live" : "camera-hidden"}`}><div className="live-character-card"><div className="live-character-picture">{member.foto_url ? <Image src={member.foto_url} alt={`Retrato de ${member.nome ?? "personagem"}`} fill sizes="20vw" unoptimized /> : <Icon name={avatar} />}</div><div className="live-character-data"><strong title={member.nome ?? "Sem personagem"}>{member.nome ?? "Sem personagem"}</strong><div className="live-character-stat life"><span style={{ width: `${statPercent(member.vida)}%` }} /><b><small>PV</small>{member.vida}</b></div><div className="live-character-stat sanity"><span style={{ width: `${statPercent(member.sanidade)}%` }} /><b><small>SAN</small>{member.sanidade}</b></div></div></div></div>;
}

function Stage({ session, members, masterUserId }: { session: Session; members: Member[]; masterUserId: string }) {
  const slots = Array.from({ length: 7 }, (_, index) => members[index]);
  return <article className="live-stage"><div className="live-stage-bar"><span>Palco da mesa</span><div className="live-stage-actions"><small>Grade</small><button className="live-fullscreen" type="button" onClick={(event) => void event.currentTarget.closest(".live-stage")?.requestFullscreen()} aria-label="Abrir palco em tela cheia">⛶</button></div></div><div className="live-grid-stage"><img className="live-grid-art" src="/grade.png" alt="Moldura da transmissão" /><div className="live-tabletop">{session.layout.tabletop_stream_id ? <iframe title="Tabletop" src={tabletopViewUrl(session.layout.tabletop_stream_id)} allow={allow} /> : <div className="live-tabletop-empty"><Icon name="mapa" /><span>Tabletop aguardando conexão do mestre</span></div>}</div><div className="live-camera-slots">{slots.map((member, index) => <div className="live-camera-slot" key={member?.user_id ?? index}>{member ? <>{!member.camera_hidden && <iframe title={`Câmera de ${member.nome ?? "participante"}`} src={viewUrl(member.stream_id)} allow={allow} />}{member.user_id !== masterUserId && <CharacterPortrait member={member} cameraLive={!member.camera_hidden} />}{member.user_id === masterUserId && member.camera_hidden && <div className="live-master-camera-off"><Icon name="selo" /><span>Câmera do mestre oculta</span></div>}{member.microphone_muted && <span className="live-muted-badge">MUDO</span>}</> : <span>Aguardando</span>}</div>)}</div></div></article>;
}

export default function CampaignSessionRoom({ campaignId, userId, masterUserId, isMaster }: { campaignId: string; userId: string; masterUserId: string; isMaster: boolean }) {
  ...
  const refresh = async () => {
    ...
    if (activeSession) {
      const [{ data: participant }, { data: sessionRoster }] = await Promise.all([
        supabase.from("campaign_session_participants").select("stream_id").eq("session_id", activeSession.id).eq("user_id", userId).is("left_at", null).maybeSingle(),
        supabase.rpc("session_roster", { target_session_id: activeSession.id }),
      ]);
      if (participant?.stream_id) setStreamId(participant.stream_id);
      setRoster((sessionRoster ?? []) as RosterMember[]);
    } else {
      setRoster([]);
    }
  };
  ...
  const rosterByUser = new Map(roster.map((entry) => [entry.user_id, entry]));
  const stageMembers = members.map((member) => { const sheet = rosterByUser.get(member.user_id); return sheet?.character_id ? { ...member, nome: sheet.nome ?? member.nome, avatar: sheet.avatar ?? member.avatar, foto_url: sheet.foto_url ?? member.foto_url } : member; });
  return <section className="live-room"><style>{`.live-shield-roster>a{width:100%;display:flex;align-items:center;gap:.55rem;border:1px solid rgba(160,42,42,.42);background:#130404;color:#eadada;padding:.48rem;text-align:left;text-decoration:none}.live-shield-roster>a:hover,.live-open-sheet:hover{border-color:#d94a4a;background:#2c0707}.live-open-sheet{width:100%;display:block;margin-top:.75rem;border:1px solid rgba(180,48,48,.55);background:#210505;color:#f0dada;padding:.65rem;text-align:center;text-decoration:none;font:600 .68rem Georgia,serif;letter-spacing:.07em;text-transform:uppercase}.live-master-camera-off{height:100%;display:grid;place-items:center;align-content:center;gap:.4rem;background:radial-gradient(circle,#1b2422,#030706 72%);color:#8f9a95;font-size:.7rem;text-align:center}.live-master-camera-off>i{color:#b63c38;font-size:1.6rem}`}</style><header className="live-top"><div><p className="live-kicker">Sessão da campanha</p><h2>{isMaster ? "Escudo do Mestre" : "Sala da campanha"}</h2></div></header><div className={`live-layout ${isMaster ? "master" : "player"}`}><Stage session={session} members={stageMembers} masterUserId={masterUserId} /><aside className="live-sheet"><p className="live-kicker">{isMaster ? "Escudo do Mestre" : "Sua ficha"}</p>{isMaster ? <><h3>Fichas em sessão</h3><p className="live-help">Abra uma ficha para alterar Vida e Sanidade durante a mesa.</p><div className="live-shield-roster">{roster.filter(entry => entry.character_id).map(entry => <a key={entry.user_id} href={`/campanhas/${campaignId}/sessao/${session.id}/ficha/${entry.character_id}`} target="_blank" rel="noopener noreferrer"><span className="live-shield-avatar">{entry.foto_url ? <Image src={entry.foto_url} alt="" fill sizes="36px" unoptimized /> : <Icon name={entry.avatar && entry.avatar in ICONS ? entry.avatar as IconName : "usuario"} />}</span><span><strong>{entry.nome ?? "Personagem"}</strong><small>PV {entry.vida} · SAN {entry.sanidade}</small></span></a>)}{!roster.some(entry => entry.character_id) && <p className="live-help">As fichas aparecerão quando os jogadores entrarem com a câmera.</p>}</div></> : <><h3>{character?.nome ?? "Sem personagem"}</h3><p className="live-help">Sua ficha controla os valores exibidos no portrait.</p>{character && <a className="live-open-sheet" href={`/campanhas/${campaignId}/sessao/${session.id}/ficha/${character.id}`} target="_blank" rel="noopener noreferrer">Abrir ficha da sessão</a>}</>}{isMaster && <div className="live-master-sources"><p className="live-kicker">Fontes do tabletop</p><button disabled={sourceBusy} type="button" onClick={() => void openMasterSource("obs")}>Tabletop pelo OBS</button><button disabled={sourceBusy} type="button" onClick={() => void openMasterSource("screen")}>Compartilhar tela</button></div>}</aside></div><section className="live-controls"><div><p className="live-kicker">Sua câmera</p><p className="live-help">Entre para reservar seu quadro na grade.</p></div><button className="live-primary" disabled={busy} onClick={() => void enter()}>{streamId ? "Reconectar câmera" : "Entrar com câmera"}</button></section><SessionMusicPlayer music={session.layout.music ?? null} isMaster={isMaster} onUpdate={updateMusic} />{notice && <p className="live-notice">{notice}</p>}</section>;
}
