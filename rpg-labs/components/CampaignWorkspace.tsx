...
type Member = { user_id: string; role: "master" | "player"; character_id?: string | null; character_name?: string | null; character_avatar?: string | null; character_photo_url?: string | null };
...
export default function CampaignWorkspace({ campaignId }: { campaignId: string }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setUserId(user.id);
      const [campaignResult, membersResult, characterRosterResult] = await Promise.all([
        ...
        supabase.rpc("campaign_character_roster", { target_campaign_id: campaignId }),
      ]);
      ...
      const basicMembers = (membersResult.data ?? []) as Member[];
      setMembers(characterRosterResult.error ? basicMembers : (characterRosterResult.data ?? basicMembers) as Member[]);
    };
    void load();
  }, [campaignId, router]);
  ...
  const tabs: { id: Tab; label: string }[] = [{ id: "visao", label: "Visão geral" }, { id: "membros", label: "Personagens" }, { id: "sessoes", label: "Sessões" }, ...(isMaster ? [{ id: "editar" as Tab, label: "Editar campanha" }] : [])];
